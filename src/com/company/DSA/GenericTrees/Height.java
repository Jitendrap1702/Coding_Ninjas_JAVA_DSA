...
public class Height {
    ...
    // height of generic tree = 1(height of root) + height of the child( with greatest height )
    public static int getHeight(TreeNode<Integer> root){
        ...
        // childHeight will be the greatest child height in all child
        int childHeight = 0;
        for (TreeNode<Integer> i : root.children){
            int currentChildHeight = getHeight(i);
            if (currentChildHeight > childHeight){
                childHeight = currentChildHeight;
            }
        }
        return 1 + childHeight; // 1 is height of root node
    }
